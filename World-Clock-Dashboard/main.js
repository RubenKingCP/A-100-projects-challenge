...
const list1 = document.querySelector("#clock1-list");
...
const list2 = document.querySelector("#clock2-list");
...
const list3 = document.querySelector("#clock3-list");
...
const list4 = document.querySelector("#clock4-list");
...
const objLength = Object.keys(data.countries).length;




function setDate(secondHand, minsHand, hourHand, difference) {
    const now = new Date();
...
function firstClock() {
    setDate(secondHand1, minsHand1, hourHand1);
}

function secondClock() {
    setDate(secondHand2, minsHand2, hourHand2);
}

function thirdClock() {
    setDate(secondHand3, minsHand3, hourHand3);
}

function fourthClock() {
    setDate(secondHand4, minsHand4, hourHand4);
}

setInterval(firstClock, 1000);
setInterval(secondClock, 1000);
setInterval(thirdClock, 1000);
setInterval(fourthClock, 1000);
...
function dropdownPrep1() {
    let holder = [list1, list2 , list3, list4];
    for (let x = 0; x < 4;x++) {
        for (let i = 0;i < objLength; i++) {
            let opt = document.createElement("option");
            opt.innerHTML = data.countries[i].name;
            opt.value = data.countries[i].timezone_offset;
            holder[x].appendChild(opt);
        }
    }
}
